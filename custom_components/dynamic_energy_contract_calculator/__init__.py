...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        saldering_map = hass.data[DOMAIN].get("saldering")
        if isinstance(saldering_map, dict):
            saldering_map.pop(entry.entry_id, None)
            if not saldering_map:
                hass.data[DOMAIN].pop("saldering")
        _LOGGER.debug("Successfully unloaded entry %s", entry.entry_id)
        remaining = [
            k
            for k in hass.data[DOMAIN]
            if k not in ("services_registered", "entities", "saldering")
        ]
